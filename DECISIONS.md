...
## 3. LLM Provider: OpenRouter (NVIDIA Nemotron 3 Ultra)

**Why:**
- Access to the free tier of the high-performance `nvidia/nemotron-3-ultra-550b-a55b:free` model (and others) with generous API ceilings.
- Provides robust reasoning, excellent structure enforcement, and huge context windows.
- Decouples API client details from vendor-specific libraries (by using a standard unified completion interface).

**Alternatives Considered:**
- Groq (Llama 3.3 70B) — extremely fast, fully supported as an alternative via `LLM_PROVIDER=groq`.
- Google Gemini — fully supported as an alternative via `LLM_PROVIDER=gemini`.
- OpenAI GPT-4o — best quality, but costs money.

**Trade-offs:**
- Large model responses can occasionally take longer compared to hyper-optimized inference engines like Groq. We mitigated this by setting the connection timeout to 120s.
- Not all models on OpenRouter support `response_format: { type: "json_object" }` natively. To handle this, we removed the explicit `response_format` JSON parameter, updated the system prompt to strongly enforce raw JSON output, and added a markdown wrapper stripper on the backend to guarantee parseable JSON responses.
